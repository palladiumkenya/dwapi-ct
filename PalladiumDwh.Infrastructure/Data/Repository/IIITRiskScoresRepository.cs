...
namespace PalladiumDwh.Infrastructure.Data.Repository
{
    ...
    public class IITRiskScoresRepository : GenericRepository<IITRiskScoresExtract>, IIITRiskScoresRepository
    {
        private readonly DwapiCentralContext _context;
        public IITRiskScoresRepository(DwapiCentralContext context) : base(context)
        {
            _context = context;
        }
        ...
        public void Sync(Guid patientId, IEnumerable<IITRiskScoresExtract> extracts)
        {
            Clear(patientId);
            Insert(extracts);
            CommitChanges();
    ...
    public void ClearNew(Guid patientId)
      {
        string sql = "DELETE FROM IITRiskScoresExtract WHERE PatientId = @PatientId";
        _context.GetConnection().Execute(sql, new { PatientId = patientId });
      }

      public void SyncNew(Guid patientId, IEnumerable<IITRiskScoresExtract> extracts)
      {
        ClearNew(patientId);
        ...
        public void SyncNewPatients(IEnumerable<IITRiskScoresProfile> profiles, IFacilityRepository facilityRepository,
            List<Guid> facIds, IActionRegisterRepository actionRegisterRepository)
        {
            ...
            var updatedProfiles = new List<IITRiskScoresProfile>();
            ...
            {
                var facilityUpdatedProfiles = new List<IITRiskScoresProfile>();
                //sync facility
            ...
            foreach (var IITRiskScoresProfile in updatedProfiles)
            {
                IITRiskScoresProfile.GenerateRecords(IITRiskScoresProfile.PatientInfo.Id);
            }
        ...
        public void SyncNew(List<IITRiskScoresProfile> profiles, IActionRegisterRepository actionRegisterRepository)
        {

            var extracts = new List<IITRiskScoresExtract>();
            var action = "DELETE";
            var area = $"{nameof(IITRiskScoresExtract)}";
            ...
            if (profiles.Any())
            {
                ...
                var sql = $@"  DELETE FROM {nameof(IITRiskScoresExtract)} 

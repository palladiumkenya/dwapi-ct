...
namespace PalladiumDwh.Infrastructure.Data.Repository
{
    ...
    public class CancerScreeningRepository : GenericRepository<CancerScreeningExtract>, ICancerScreeningRepository
    {
        private readonly DwapiCentralContext _context;
        public CancerScreeningRepository(DwapiCentralContext context) : base(context)
        {
            _context = context;
        }
        ...
        public void Sync(Guid patientId, IEnumerable<CancerScreeningExtract> extracts)
        {
            Clear(patientId);
            Insert(extracts);
            CommitChanges();
    ...
    public void ClearNew(Guid patientId)
      {
        string sql = "DELETE FROM CancerScreeningExtract WHERE PatientId = @PatientId";
        _context.GetConnection().Execute(sql, new { PatientId = patientId });
      }

      public void SyncNew(Guid patientId, IEnumerable<CancerScreeningExtract> extracts)
      {
        ClearNew(patientId);
        ...
        public void SyncNewPatients(IEnumerable<CancerScreeningProfile> profiles, IFacilityRepository facilityRepository,
            List<Guid> facIds, IActionRegisterRepository actionRegisterRepository)
        {
            ...
            var updatedProfiles = new List<CancerScreeningProfile>();
            ...
            {
                var facilityUpdatedProfiles = new List<CancerScreeningProfile>();
                //sync facility
            ...
            foreach (var CancerScreeningProfile in updatedProfiles)
            {
                CancerScreeningProfile.GenerateRecords(CancerScreeningProfile.PatientInfo.Id);
            }
        ...
        public void SyncNew(List<CancerScreeningProfile> profiles, IActionRegisterRepository actionRegisterRepository)
        {

            var extracts = new List<CancerScreeningExtract>();
            var action = "DELETE";
            var area = $"{nameof(CancerScreeningExtract)}";
            ...
            if (profiles.Any())
            {
                ...
                var sql = $@"  DELETE FROM {nameof(CancerScreeningExtract)} 

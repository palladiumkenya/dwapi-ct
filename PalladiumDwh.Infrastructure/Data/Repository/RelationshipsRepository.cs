...
namespace PalladiumDwh.Infrastructure.Data.Repository
{
    ...
    public class RelationshipsRepository : GenericRepository<RelationshipsExtract>, IRelationshipsRepository
    {
        private readonly DwapiCentralContext _context;
        public RelationshipsRepository(DwapiCentralContext context) : base(context)
        {
            _context = context;
        }
        ...
        public void Sync(Guid patientId, IEnumerable<RelationshipsExtract> extracts)
        {
            Clear(patientId);
            Insert(extracts);
            CommitChanges();
    ...
    public void ClearNew(Guid patientId)
      {
        string sql = "DELETE FROM RelationshipsExtract WHERE PatientId = @PatientId";
        _context.GetConnection().Execute(sql, new { PatientId = patientId });
      }

      public void SyncNew(Guid patientId, IEnumerable<RelationshipsExtract> extracts)
      {
        ClearNew(patientId);
        ...
        public void SyncNewPatients(IEnumerable<RelationshipsProfile> profiles, IFacilityRepository facilityRepository,
            List<Guid> facIds, IActionRegisterRepository actionRegisterRepository)
        {
            ...
            var updatedProfiles = new List<RelationshipsProfile>();
            ...
            {
                var facilityUpdatedProfiles = new List<RelationshipsProfile>();
                //sync facility
            ...
            foreach (var RelationshipsProfile in updatedProfiles)
            {
                RelationshipsProfile.GenerateRecords(RelationshipsProfile.PatientInfo.Id);
            }
        ...
        public void SyncNew(List<RelationshipsProfile> profiles, IActionRegisterRepository actionRegisterRepository)
        {

            var extracts = new List<RelationshipsExtract>();
            var action = "DELETE";
            var area = $"{nameof(RelationshipsExtract)}";
            ...
            if (profiles.Any())
            {
                ...
                var sql = $@"  DELETE FROM {nameof(RelationshipsExtract)} 

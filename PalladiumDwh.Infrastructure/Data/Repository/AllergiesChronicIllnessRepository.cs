...
namespace PalladiumDwh.Infrastructure.Data.Repository
{
    ...
    public class AllergiesChronicIllnessRepository : GenericRepository<AllergiesChronicIllnessExtract>, IAllergiesChronicIllnessRepository
    {
        private readonly DwapiCentralContext _context;
        public AllergiesChronicIllnessRepository(DwapiCentralContext context) : base(context)
        {
            _context = context;
        }
        ...
        public void Sync(Guid patientId, IEnumerable<AllergiesChronicIllnessExtract> extracts)
        {
            Clear(patientId);
            Insert(extracts);
            CommitChanges();
    ...
    public void ClearNew(Guid patientId)
      {
        string sql = "DELETE FROM AllergiesChronicIllnessExtract WHERE PatientId = @PatientId";
        _context.GetConnection().Execute(sql, new { PatientId = patientId });
      }

      public void SyncNew(Guid patientId, IEnumerable<AllergiesChronicIllnessExtract> extracts)
      {
        ClearNew(patientId);
        ...
        public void SyncNewPatients(IEnumerable<AllergiesChronicIllnessProfile> profiles, IFacilityRepository facilityRepository,
            List<Guid> facIds, IActionRegisterRepository actionRegisterRepository)
        {
            ...
            var updatedProfiles = new List<AllergiesChronicIllnessProfile>();
            ...
            {
                var facilityUpdatedProfiles = new List<AllergiesChronicIllnessProfile>();
                //sync facility
            ...
            foreach (var AllergiesChronicIllnessProfile in updatedProfiles)
            {
                AllergiesChronicIllnessProfile.GenerateRecords(AllergiesChronicIllnessProfile.PatientInfo.Id);
            }
        ...
        public void SyncNew(List<AllergiesChronicIllnessProfile> profiles, IActionRegisterRepository actionRegisterRepository)
        {

            var extracts = new List<AllergiesChronicIllnessExtract>();
            var action = "DELETE";
            var area = $"{nameof(AllergiesChronicIllnessExtract)}";
            ...
            if (profiles.Any())
            {
                ...
                var sql = $@"  DELETE FROM {nameof(AllergiesChronicIllnessExtract)} 

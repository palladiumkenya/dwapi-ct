...
namespace PalladiumDwh.Infrastructure.Data.Repository
{
    ...
    public class DepressionScreeningRepository : GenericRepository<DepressionScreeningExtract>, IDepressionScreeningRepository
    {
        private readonly DwapiCentralContext _context;
        public DepressionScreeningRepository(DwapiCentralContext context) : base(context)
        {
            _context = context;
        }
        ...
        public void Sync(Guid patientId, IEnumerable<DepressionScreeningExtract> extracts)
        {
            Clear(patientId);
            Insert(extracts);
            CommitChanges();
    ...
    public void ClearNew(Guid patientId)
      {
        string sql = "DELETE FROM DepressionScreeningExtract WHERE PatientId = @PatientId";
        _context.GetConnection().Execute(sql, new { PatientId = patientId });
      }

      public void SyncNew(Guid patientId, IEnumerable<DepressionScreeningExtract> extracts)
      {
        ClearNew(patientId);
        _context.GetConnection().BulkInsert(extracts);
      }

      public void SyncNewPatients(IEnumerable<DepressionScreeningProfile> profiles, IFacilityRepository facilityRepository,
          List<Guid> facIds)
      {
          var updates = new List<PatientExtract>();
          var inserts = new List<PatientExtract>();
          var updatedProfiles = new List<DepressionScreeningProfile>();
          ...
          {
              var facilityUpdatedProfiles = new List<DepressionScreeningProfile>();
              //sync facility
          ...
          {
              DepressionScreeningProfile.GenerateRecords(DepressionScreeningProfile.PatientInfo.Id);
          }

          SyncNew(updatedProfiles);
      }

      public void SyncNew(IEnumerable<DepressionScreeningProfile> profiles)
        {
            var ids = new List<string>();
            var extracts = new List<DepressionScreeningExtract>();
            ...
            if (ids.Count > 0)
            {
                ...
                var sql = $@" DELETE FROM DepressionScreeningExtract WHERE (PatientId In ({allIds}))";
                try
                {

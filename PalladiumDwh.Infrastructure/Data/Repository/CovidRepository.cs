...
namespace PalladiumDwh.Infrastructure.Data.Repository
{
    ...
    public class CovidRepository : GenericRepository<CovidExtract>, ICovidRepository
    {
        private readonly DwapiCentralContext _context;
        public CovidRepository(DwapiCentralContext context) : base(context)
        {
            _context = context;
        }
        ...
        public void Sync(Guid patientId, IEnumerable<CovidExtract> extracts)
        {
            Clear(patientId);
            Insert(extracts);
            CommitChanges();
    ...
    public void ClearNew(Guid patientId)
      {
        string sql = "DELETE FROM CovidExtract WHERE PatientId = @PatientId";
        _context.GetConnection().Execute(sql, new { PatientId = patientId });
      }

      public void SyncNew(Guid patientId, IEnumerable<CovidExtract> extracts)
      {
        ClearNew(patientId);
        ...
        public void SyncNewPatients(IEnumerable<CovidProfile> profiles, IFacilityRepository facilityRepository,
            List<Guid> facIds, IActionRegisterRepository actionRegisterRepository)
        {
            ...
            var updatedProfiles = new List<CovidProfile>();
            ...
            {
                var facilityUpdatedProfiles = new List<CovidProfile>();
                //sync facility
            ...
            foreach (var CovidProfile in updatedProfiles)
            {
                CovidProfile.GenerateRecords(CovidProfile.PatientInfo.Id);
            }
        ...
        public void SyncNew(List<CovidProfile> profiles, IActionRegisterRepository actionRegisterRepository)
        {

            var extracts = new List<CovidExtract>();
            var action = "DELETE";
            var area = $"{nameof(CovidExtract)}";
            ...
            if (profiles.Any())
            {
                ...
                var sql = $@"  DELETE FROM {nameof(CovidExtract)} 

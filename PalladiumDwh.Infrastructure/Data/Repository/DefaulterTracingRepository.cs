...
namespace PalladiumDwh.Infrastructure.Data.Repository
{
    ...
    public class DefaulterTracingRepository : GenericRepository<DefaulterTracingExtract>, IDefaulterTracingRepository
    {
        private readonly DwapiCentralContext _context;
        public DefaulterTracingRepository(DwapiCentralContext context) : base(context)
        {
            _context = context;
        }
        ...
        public void Sync(Guid patientId, IEnumerable<DefaulterTracingExtract> extracts)
        {
            Clear(patientId);
            Insert(extracts);
            CommitChanges();
    ...
    public void ClearNew(Guid patientId)
      {
        string sql = "DELETE FROM DefaulterTracingExtract WHERE PatientId = @PatientId";
        _context.GetConnection().Execute(sql, new { PatientId = patientId });
      }

      public void SyncNew(Guid patientId, IEnumerable<DefaulterTracingExtract> extracts)
      {
        ClearNew(patientId);
        ...
        public void SyncNewPatients(IEnumerable<DefaulterTracingProfile> profiles, IFacilityRepository facilityRepository,
            List<Guid> facIds, IActionRegisterRepository actionRegisterRepository)
        {
            ...
            var updatedProfiles = new List<DefaulterTracingProfile>();
            ...
            {
                var facilityUpdatedProfiles = new List<DefaulterTracingProfile>();
                //sync facility
            ...
            foreach (var DefaulterTracingProfile in updatedProfiles)
            {
                DefaulterTracingProfile.GenerateRecords(DefaulterTracingProfile.PatientInfo.Id);
            }
        ...
        public void SyncNew(List<DefaulterTracingProfile> profiles, IActionRegisterRepository actionRegisterRepository)
        {

            var extracts = new List<DefaulterTracingExtract>();
            var action = "DELETE";
            var area = $"{nameof(DefaulterTracingExtract)}";
            ...
            if (profiles.Any())
            {
                ...
                var sql = $@"  DELETE FROM {nameof(DefaulterTracingExtract)} 

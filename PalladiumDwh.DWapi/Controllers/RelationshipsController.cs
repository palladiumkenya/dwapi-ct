...
namespace PalladiumDwh.DWapi.Controllers
{
    public class RelationshipsController : ApiController
    {
        ...
        private readonly string _gateway = typeof(RelationshipsProfile).Name.ToLower();
        private readonly string _gatewayBatch;
        private readonly IMessengerScheduler _messengerScheduler;
        private readonly IMediator _mediator;
        public RelationshipsController(IMessagingSenderService messagingService,IMessengerScheduler messengerScheduler, IMediator mediator)
        {
        ...
        /*
        public async Task<HttpResponseMessage> Post([FromBody] RelationshipsProfile patientProfile)
        {
            if (null != patientProfile)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    Log.Error(nameof(RelationshipsProfile),ex);
                    Log.Error(new string('*',30));
                    return Request.CreateErrorResponse(HttpStatusCode.BadRequest, ex);
                }
            }
            return Request.CreateErrorResponse(HttpStatusCode.BadRequest, new HttpError($"The expected '{new RelationshipsProfile().GetType().Name}' is null"));
        }
        */

        [Route("api/v2/Relationships")]
        public async Task<HttpResponseMessage> PostBatch([FromBody] List<RelationshipsProfile> patientProfile)
        {
            ...
            return Request.CreateErrorResponse(HttpStatusCode.BadRequest,
                new HttpError($"The expected '{new RelationshipsProfile().GetType().Name}' is null"));
        }

        [Route("api/v3/Relationships")]
        public async Task<HttpResponseMessage> PostBatchNew([FromBody] RelationshipsSourceBag sourceBag)
        {
            ...
            if (null != sourceBag && sourceBag.Extracts.Any())
            {
                ...
                try
                {
                    ...
                    if (sourceBag.HasJobId)
                    {
                        jobId = BatchJob.ContinueBatchWith(sourceBag.JobId,
                            x => { x.Enqueue(() => Send($"{sourceBag}", new SyncRelationships(sourceBag))); },$"{sourceBag}");
                    }
                    else
                    {
                        jobId = BatchJob.StartNew(x =>
                        {
                            x.Enqueue(() => Send($"{sourceBag}", new SyncRelationships(sourceBag)));
                        },$"{sourceBag}");
                    }
                ...
                {
                    Log.Error(new string('*', 30));
                    Log.Error(nameof(RelationshipsSourceBag), ex);
                    Log.Error(new string('*', 30));
                    return Request.CreateErrorResponse(HttpStatusCode.BadRequest, ex);
                }
            }

...
{
    public class OvcController : ApiController
    {
        private static readonly ILog Log = LogManager.GetLogger(MethodBase.GetCurrentMethod().DeclaringType);
        private readonly IMessagingSenderService _messagingService;
        private readonly string _gateway = typeof(OvcProfile).Name.ToLower();
        private readonly string _gatewayBatch;
        private readonly IMessengerScheduler _messengerScheduler;
        private readonly IMediator _mediator;
        public OvcController(IMessagingSenderService messagingService,IMessengerScheduler messengerScheduler, IMediator mediator)
        {
        ...
        // public async Task<HttpResponseMessage> Post([FromBody] OvcProfile patientProfile)
        // {
        ...
        //             Log.Error(nameof(OvcProfile),ex);
        //             Log.Error(new string('*',30));
        //             return Request.CreateErrorResponse(HttpStatusCode.BadRequest, ex);
        //         }
        //     }
        //     return Request.CreateErrorResponse(HttpStatusCode.BadRequest, new HttpError($"The expected '{new OvcProfile().GetType().Name}' is null"));
        // }

        [Route("api/v2/Ovc")]
        public async Task<HttpResponseMessage> PostBatch([FromBody] List<OvcProfile> patientProfile)
        {
            ...
            return Request.CreateErrorResponse(HttpStatusCode.BadRequest,
                new HttpError($"The expected '{new OvcProfile().GetType().Name}' is null"));
        }

        [Route("api/v3/Ovc")]
        public async Task<HttpResponseMessage> PostBatchNew([FromBody] OvcSourceBag sourceBag)
        {
                ...
                try
                {
                    ...
                    if (sourceBag.HasJobId)
                    {
                        jobId = BatchJob.ContinueBatchWith(sourceBag.JobId,
                            x => { x.Enqueue(() => Send($"{sourceBag}", new SyncOvc(sourceBag))); },$"{sourceBag}");
                    }
                    else
                    {
                         jobId = BatchJob.StartNew(x =>
                        {
                            x.Enqueue(() => Send($"{sourceBag}", new SyncOvc(sourceBag)));
                        },$"{sourceBag}");
                    }

...
namespace PalladiumDwh.DWapi.Controllers
{
    public class PatientStatusController : ApiController
    {
        ...
        private readonly string _gateway = typeof(PatientStatusProfile).Name.ToLower();
        private readonly string _gatewayBatch;
        private readonly IMessengerScheduler _messengerScheduler;
        private readonly IMediator _mediator;
        public PatientStatusController(IMessagingSenderService messagingService,IMessengerScheduler messengerScheduler, IMediator mediator)
        {
        ...
        // public async Task<HttpResponseMessage> Post([FromBody] PatientStatusProfile patientProfile)
        // {
        ...
        //             Log.Error(nameof(PatientStatusProfile),ex);
        //             Log.Error(new string('*',30));
        //             return Request.CreateErrorResponse(HttpStatusCode.BadRequest, ex);
        //         }
        //     }
        //     return Request.CreateErrorResponse(HttpStatusCode.BadRequest, new HttpError($"The expected '{new PatientStatusProfile().GetType().Name}' is null"));
        // }

        [Route("api/v2/PatientStatus")]
        public async Task<HttpResponseMessage> PostBatch([FromBody] List<PatientStatusProfile> patientProfile)
        {
            ...
            return Request.CreateErrorResponse(HttpStatusCode.BadRequest,
                new HttpError($"The expected '{new PatientStatusProfile().GetType().Name}' is null"));
        }

        [Route("api/v3/PatientStatus")]
        public async Task<HttpResponseMessage> PostBatchNew([FromBody] StatusSourceBag sourceBag)
        {
                ...
                try
                {
                    ...
                    if (sourceBag.HasJobId)
                    {
                        jobId = BatchJob.ContinueBatchWith(sourceBag.JobId,
                            x => { x.Enqueue(() => Send($"{sourceBag}", new SyncStatus(sourceBag))); },$"{sourceBag}");
                    }
                    else
                    {
                        jobId = BatchJob.StartNew(x =>
                        {
                            x.Enqueue(() => Send($"{sourceBag}", new SyncStatus(sourceBag)));
                        },$"{sourceBag}");
                    }

...
{
    public class ContactListingController : ApiController
    {
        private static readonly ILog Log = LogManager.GetLogger(MethodBase.GetCurrentMethod().DeclaringType);
        private readonly IMessagingSenderService _messagingService;
        private readonly string _gateway = typeof(ContactListingProfile).Name.ToLower();
        ...
        public async Task<HttpResponseMessage> PostBatch([FromBody] List<ContactListingProfile> patientProfile)
        {
            ...
            return Request.CreateErrorResponse(HttpStatusCode.BadRequest,
                new HttpError($"The expected '{new ContactListingProfile().GetType().Name}' is null"));
        }
                ...
                try
                {
                    ...
                    if (sourceBag.HasJobId)
                    {
                        jobId = BatchJob.ContinueBatchWith(sourceBag.JobId,
                            x => { x.Enqueue(() => Send($"{sourceBag}", new SyncContactListing(sourceBag))); },$"{sourceBag}");
                    }
                    else
                    {

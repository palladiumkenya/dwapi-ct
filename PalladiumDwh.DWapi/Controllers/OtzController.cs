...
namespace PalladiumDwh.DWapi.Controllers
{
    public class OtzController : ApiController
    {
        ...
        public OtzController(IMessagingSenderService messagingService,IMessengerScheduler messengerScheduler, IMediator mediator)
        {
        ...
        [Route("api/v3/Otz")]
        public async Task<HttpResponseMessage> PostBatchNew([FromBody] OtzSourceBag sourceBag)
        {
                ...
                try
                {
                    ...
                    if (sourceBag.HasJobId)
                    {
                        jobId = BatchJob.ContinueBatchWith(sourceBag.JobId,
                            x => { x.Enqueue(() => Send($"{sourceBag}", new SyncOtz(sourceBag))); },$"{sourceBag}");
                    }
                    else
                    {
                        jobId = BatchJob.StartNew(x =>
                        {
                            x.Enqueue(() => Send($"{sourceBag}", new SyncOtz(sourceBag)));
                        },$"{sourceBag}");
                    }

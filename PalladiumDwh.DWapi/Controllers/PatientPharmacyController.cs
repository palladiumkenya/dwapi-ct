...
namespace PalladiumDwh.DWapi.Controllers
{
    public class PatientPharmacyController : ApiController
    {
        ...
        public async Task<HttpResponseMessage> PostBatchNew([FromBody] PharmacySourceBag sourceBag)
        {
                ...
                try
                {
                    ...
                    if (sourceBag.HasJobId)
                    {
                        jobId = BatchJob.ContinueBatchWith(sourceBag.JobId,
                            x => { x.Enqueue(() => Send($"{sourceBag}", new SyncPharmacy(sourceBag))); },$"{sourceBag}");
                    }
                    else
                    {

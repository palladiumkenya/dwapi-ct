...
{
    public class IITRiskScoresController : ApiController
    {
        private static readonly ILog Log = LogManager.GetLogger(MethodBase.GetCurrentMethod().DeclaringType);
        private readonly IMessagingSenderService _messagingService;
        private readonly string _gateway = typeof(IITRiskScoresProfile).Name.ToLower();
        private readonly string _gatewayBatch;
        private readonly IMessengerScheduler _messengerScheduler;
        private readonly IMediator _mediator;
        public IITRiskScoresController(IMessagingSenderService messagingService,IMessengerScheduler messengerScheduler, IMediator mediator)
        {
        ...
        /*
        public async Task<HttpResponseMessage> Post([FromBody] IITRiskScoresProfile patientProfile)
        {
            if (null != patientProfile)
            {
                ...
                catch (Exception ex)
                {
                    ...
                    Log.Error(nameof(IITRiskScoresProfile),ex);
                    Log.Error(new string('*',30));
                    return Request.CreateErrorResponse(HttpStatusCode.BadRequest, ex);
                }
            }
            return Request.CreateErrorResponse(HttpStatusCode.BadRequest, new HttpError($"The expected '{new IITRiskScoresProfile().GetType().Name}' is null"));
        }
        */

        [Route("api/v2/IITRiskScores")]
        public async Task<HttpResponseMessage> PostBatch([FromBody] List<IITRiskScoresProfile> patientProfile)
        {
            ...
            return Request.CreateErrorResponse(HttpStatusCode.BadRequest,
                new HttpError($"The expected '{new IITRiskScoresProfile().GetType().Name}' is null"));
        }

        [Route("api/v3/IITRiskScores")]
        public async Task<HttpResponseMessage> PostBatchNew([FromBody] IITRiskScoresSourceBag sourceBag)
        {
            ...
            if (null != sourceBag && sourceBag.Extracts.Any())
            {
                ...
                try
                {
                    ...
                    if (sourceBag.HasJobId)
                    {
                        jobId = BatchJob.ContinueBatchWith(sourceBag.JobId,
                            x => { x.Enqueue(() => Send($"{sourceBag}", new SyncIITRiskScores(sourceBag))); },$"{sourceBag}");
                    }
                    else
                    {
                        jobId = BatchJob.StartNew(x =>
                        {
                            x.Enqueue(() => Send($"{sourceBag}", new SyncIITRiskScores(sourceBag)));
                        },$"{sourceBag}");
                    }
                ...
                {
                    Log.Error(new string('*', 30));
                    Log.Error(nameof(IITRiskScoresSourceBag), ex);
                    Log.Error(new string('*', 30));
                    return Request.CreateErrorResponse(HttpStatusCode.BadRequest, ex);
                }
            }

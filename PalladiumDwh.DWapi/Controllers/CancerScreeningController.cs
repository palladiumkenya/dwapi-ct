...
{
    public class CancerScreeningController : ApiController
    {
        private static readonly ILog Log = LogManager.GetLogger(MethodBase.GetCurrentMethod().DeclaringType);
        private readonly IMessagingSenderService _messagingService;
        private readonly string _gateway = typeof(CancerScreeningProfile).Name.ToLower();
        private readonly string _gatewayBatch;
        private readonly IMessengerScheduler _messengerScheduler;
        private readonly IMediator _mediator;
        public CancerScreeningController(IMessagingSenderService messagingService,IMessengerScheduler messengerScheduler, IMediator mediator)
        {
        ...
        */
        ...
        public async Task<HttpResponseMessage> PostBatch([FromBody] List<CancerScreeningProfile> patientProfile)
        {
            ...
            return Request.CreateErrorResponse(HttpStatusCode.BadRequest,
                new HttpError($"The expected '{new CancerScreeningProfile().GetType().Name}' is null"));
        }

        [Route("api/v3/CancerScreening")]
        public async Task<HttpResponseMessage> PostBatchNew([FromBody] CancerScreeningSourceBag sourceBag)
        {
            ...
            if (null != sourceBag && sourceBag.Extracts.Any())
            {
                ...
                try
                {
                    ...
                    if (sourceBag.HasJobId)
                    {
                        jobId = BatchJob.ContinueBatchWith(sourceBag.JobId,
                            x => { x.Enqueue(() => Send($"{sourceBag}", new SyncCancerScreening(sourceBag))); },$"{sourceBag}");
                    }
                    else
                    {
                        jobId = BatchJob.StartNew(x =>
                        {
                            x.Enqueue(() => Send($"{sourceBag}", new SyncCancerScreening(sourceBag)));
                        },$"{sourceBag}");
                    }
                ...
                {
                    Log.Error(new string('*', 30));
                    Log.Error(nameof(CancerScreeningSourceBag), ex);
                    Log.Error(new string('*', 30));
                    return Request.CreateErrorResponse(HttpStatusCode.BadRequest, ex);
                }
            }

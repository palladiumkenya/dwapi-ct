...
namespace PalladiumDwh.DWapi.Controllers
{
    public class PatientAdverseEventsController : ApiController
    {
        ...
        private readonly string _gateway = nameof(PatientAdverseEventProfile).ToLower();
        private readonly string _gatewayBatch;
        private readonly IMessengerScheduler _messengerScheduler;
        private readonly IMediator _mediator;

        public PatientAdverseEventsController(IMessagingSenderService messagingService,IMessengerScheduler messengerScheduler, IMediator mediator)
        {
        ...
        //             Log.Error(nameof(PatientAdverseEventProfile),ex);
        //             Log.Error(new string('*',30));
        ...
        [Route("api/v2/PatientAdverseEvents")]
        ...
        public async Task<HttpResponseMessage> PostBatchNew([FromBody] AdverseEventSourceBag sourceBag)
        {
                ...
                try
                {
                    ...
                    if (sourceBag.HasJobId)
                    {
                        jobId = BatchJob.ContinueBatchWith(sourceBag.JobId,
                            x => { x.Enqueue(() => Send($"{sourceBag}", new SyncAdverseEvent(sourceBag))); },$"{sourceBag}");
                    }
                    else
                    {
                        jobId = BatchJob.StartNew(x =>
                        {
                            x.Enqueue(() => Send($"{sourceBag}", new SyncAdverseEvent(sourceBag)));
                        },$"{sourceBag}");
                    }

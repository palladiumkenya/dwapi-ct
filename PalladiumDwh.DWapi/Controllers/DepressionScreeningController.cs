...
{
    public class DepressionScreeningController : ApiController
    {
        private static readonly ILog Log = LogManager.GetLogger(MethodBase.GetCurrentMethod().DeclaringType);
        private readonly IMessagingSenderService _messagingService;
        private readonly string _gateway = typeof(DepressionScreeningProfile).Name.ToLower();
        private readonly string _gatewayBatch;
        private readonly IMessengerScheduler _messengerScheduler;
        private readonly IMediator _mediator;
        public DepressionScreeningController(IMessagingSenderService messagingService,IMessengerScheduler messengerScheduler, IMediator mediator)
        {
        ...
        // public async Task<HttpResponseMessage> Post([FromBody] DepressionScreeningProfile patientProfile)
        // {
        ...
        //             Log.Error(nameof(DepressionScreeningProfile),ex);
        //             Log.Error(new string('*',30));
        //             return Request.CreateErrorResponse(HttpStatusCode.BadRequest, ex);
        //         }
        //     }
        //     return Request.CreateErrorResponse(HttpStatusCode.BadRequest, new HttpError($"The expected '{new DepressionScreeningProfile().GetType().Name}' is null"));
        // }

        [Route("api/v2/DepressionScreening")]
        public async Task<HttpResponseMessage> PostBatch([FromBody] List<DepressionScreeningProfile> patientProfile)
        {
            ...
            return Request.CreateErrorResponse(HttpStatusCode.BadRequest,
                new HttpError($"The expected '{new DepressionScreeningProfile().GetType().Name}' is null"));
        }

        [Route("api/v3/DepressionScreening")]
        public async Task<HttpResponseMessage> PostBatchNew([FromBody] DepressionScreeningSourceBag sourceBag)
        {
                ...
                try
                {
                    ...
                    if (sourceBag.HasJobId)
                    {
                        jobId = BatchJob.ContinueBatchWith(sourceBag.JobId,
                            x => { x.Enqueue(() => Send($"{sourceBag}", new SyncDepressionScreening(sourceBag))); },$"{sourceBag}");
                    }
                    else
                    {
                        jobId = BatchJob.StartNew(x =>
                        {
                            x.Enqueue(() => Send($"{sourceBag}", new SyncDepressionScreening(sourceBag)));
                        },$"{sourceBag}");
                    }

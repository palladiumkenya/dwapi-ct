...
namespace PalladiumDwh.DWapi.Controllers
{
    public class PatientLabsController : ApiController
    {
        ...
        public PatientLabsController(IMessagingSenderService messagingService,IMessengerScheduler messengerScheduler, IMediator mediator)
        {
        ...
        public async Task<HttpResponseMessage> PostBatchNew([FromBody] LaboratorySourceBag sourceBag)
        {
                ...
                try
                {
                    ...
                    if (sourceBag.HasJobId)
                    {
                        jobId = BatchJob.ContinueBatchWith(sourceBag.JobId,
                            x => { x.Enqueue(() => Send($"{sourceBag}", new SyncLaboratory(sourceBag))); },$"{sourceBag}");
                    }
                    else
                    {
                        jobId = BatchJob.StartNew(x =>
                        {
                            x.Enqueue(() => Send($"{sourceBag}", new SyncLaboratory(sourceBag)));
                        },$"{sourceBag}");
                    }

...
{
    public class DrugAlcoholScreeningController : ApiController
    {
        private static readonly ILog Log = LogManager.GetLogger(MethodBase.GetCurrentMethod().DeclaringType);
        private readonly IMessagingSenderService _messagingService;
        private readonly string _gateway = typeof(DrugAlcoholScreeningProfile).Name.ToLower();
        private readonly string _gatewayBatch;
        private readonly IMessengerScheduler _messengerScheduler;
        private readonly IMediator _mediator;
        public DrugAlcoholScreeningController(IMessagingSenderService messagingService,IMessengerScheduler messengerScheduler, IMediator mediator)
        {
        ...
        // public async Task<HttpResponseMessage> Post([FromBody] DrugAlcoholScreeningProfile patientProfile)
        // {
        ...
        //             Log.Error(nameof(DrugAlcoholScreeningProfile),ex);
        //             Log.Error(new string('*',30));
        //             return Request.CreateErrorResponse(HttpStatusCode.BadRequest, ex);
        //         }
        //     }
        //     return Request.CreateErrorResponse(HttpStatusCode.BadRequest, new HttpError($"The expected '{new DrugAlcoholScreeningProfile().GetType().Name}' is null"));
        // }

        [Route("api/v2/DrugAlcoholScreening")]
        public async Task<HttpResponseMessage> PostBatch([FromBody] List<DrugAlcoholScreeningProfile> patientProfile)
        {
            ...
            return Request.CreateErrorResponse(HttpStatusCode.BadRequest,
                new HttpError($"The expected '{new DrugAlcoholScreeningProfile().GetType().Name}' is null"));
        }

        [Route("api/v3/DrugAlcoholScreening")]
        public async Task<HttpResponseMessage> PostBatchNew([FromBody] DrugAlcoholScreeningSourceBag sourceBag)
        {
                ...
                try
                {
                    ...
                    if (sourceBag.HasJobId)
                    {
                        jobId = BatchJob.ContinueBatchWith(sourceBag.JobId,
                            x => { x.Enqueue(() => Send($"{sourceBag}", new SyncDrugAlcoholScreening(sourceBag))); },$"{sourceBag}");
                    }
                    else
                    {
                         jobId = BatchJob.StartNew(x =>
                        {
                            x.Enqueue(() => Send($"{sourceBag}", new SyncDrugAlcoholScreening(sourceBag)));
                        },$"{sourceBag}");
                    }

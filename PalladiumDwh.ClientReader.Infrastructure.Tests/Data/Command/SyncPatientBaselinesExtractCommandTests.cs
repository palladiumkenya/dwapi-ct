...
{
    public class SyncPatientBaselinesExtractCommandTests
    {
        private DwapiRemoteContext _context;
        private ISyncPatientBaselinesExtractCommand _syncCommand;
        ...
        public void SetUp()
        {
            ...
            _context.Database.ExecuteSqlCommand("DELETE FROM EventHistory;DELETE FROM PatientBaselinesExtract;DELETE FROM TempPatientBaselinesExtract;");
        ...
        public void should_sync_patients()
        {
            ...
            var extractCommand = new LoadPatientBaselinesExtractCommand(_emrRepository);
            var extracts = extractCommand.ExecuteAsync().Result;
            var validateCommand = new ValidatePatientBaselinesExtractCommand(_emrRepository, new ValidatorRepository(_context));
            var validations = validateCommand.ExecuteAsync().Result;

            var watch = System.Diagnostics.Stopwatch.StartNew();
            _syncCommand = new SyncPatientBaselinesExtractCommand(_emrRepository);
            var totals = _syncCommand.ExecuteAsync(_dprogress).Result;

            watch.Stop();
            var tempRecords = _context.Database
                .SqlQuery<int>("SELECT COUNT(*) as NumOfRecords FROM PatientBaselinesExtract")
                .Single();

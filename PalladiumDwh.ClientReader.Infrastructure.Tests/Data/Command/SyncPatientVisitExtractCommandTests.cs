...
namespace PalladiumDwh.ClientReader.Infrastructure.Tests.Data.Command
{
    public class SyncPatientVisitExtractCommandTests
    {
        ...
        {
            var extractCommand = new LoadPatientVisitExtractDbCommand(new SqlConnection(_srcConnectionString), new SqlConnection(_connectionString), TestHelpers.GetPatientVisitsSql(top));
            extractCommand.Execute();
            var watch = System.Diagnostics.Stopwatch.StartNew();

            _syncCommand = new SyncPatientVisitExtractDbCommand(_connectionString);

...
namespace PalladiumDwh.ClientReader.Infrastructure.Tests.Data.Repository
{
    ...
    [TestFixture]
    public class TempExtractRepositoryTests
    {
        private DwapiRemoteContext _context;
        private List<TempPatientExtract> _TempPatientExtracts;
        private ITempPatientExtractRepository _repository;

        [SetUp]
        public void SetUp()
        {           
            _context = new DwapiRemoteContext(Effort.DbConnectionFactory.CreateTransient(), true);
            _TempPatientExtracts = Builder<TempPatientExtract>.CreateListOfSize(10).Build().ToList();
            foreach (var e in _TempPatientExtracts)
            {
                e.CheckError = true;
            }
            TestHelpers.CreateTestData(_context, _TempPatientExtracts);

            _repository = new TempPatientExtractRepository(_context);
        }
